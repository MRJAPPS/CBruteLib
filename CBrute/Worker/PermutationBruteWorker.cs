...
{
    /// <summary>
    /// With this class, you can perform the password generation process using PermutationBrute in parallel.
    ...
    /// </summary>
    public sealed class PermutationBruteWorker : Worker
    {
        /// <summary>
        /// Constructor of the PermutationBruteWorker class
        /// </summary>
        /// <param name="startPos">
        ...
        /// </param>
        /// <param name="maximumPasswordLength">
        /// Maximum password length. In Permutation, this parameter cannot be greater than the length of the <paramref name="test"/> parameter.
        ...
        public PermutationBruteWorker(long startPos, long endPos, int minimumPasswordLength, int maximumPasswordLength,
        ...
        /// <summary>
        /// This function is used to detect errors that may occur during the instantiation of <see cref="PermutationBruteWorker"/>
        /// </summary>
        ...
        private void ProBruteWorkerE(long startPos, long endPos, int minimumPasswordLength, int maximumPasswordLength,
            object[] test, int threadCount, delegate_CheckPassword passChecker)
        {
            PermutationBrute.PermutationBruteE(startPos, ref endPos, minimumPasswordLength, maximumPasswordLength, test);
        ...
        private void initializeBruteFoceList()
        {
            ...
            for (int threadID = 0; threadID < ThreadCount; ++threadID)
            {
                ...
                PermutationBrute PEB = new PermutationBrute(startPos, endPos, min, max, test, threadID);
                list.Add(PEB);
            }
        }
        /// <summary>
        /// This function adds parallel programming related events to <paramref name="PEB"/>.
        /// </summary>
        private void initializeBruteFoceListEvents(PermutationBrute PEB)
        {
            PEB.PasswordGenerated += (BruteForce sender, object[] pass, long generated, long total) =>
            check(this, sender, pass, generated, total);
            PEB.OnError += (BruteForce sender, Exception e) =>
            {
                bool tryAgain = false;
                ...
                if (tryAgain)
                {
                    ...
                    PermutationBrute onError_PEB = (PermutationBrute)sender;
                    onError_PEB.StartPos = onError_PEB.RealPos;
                    onError_PEB.Start(BruteForce.ErrorHandlingType.Event);
                }
                else
                {
                    errorCounter.Signal();
            ...
            PEB.OnPause += (BruteForce sender, long generated, long total) =>
            {
                OnThreadPause?.Invoke(this, sender, generated, total);
                pauseCount();
            };
            PEB.OnResume += (BruteForce sender, long generated, long total) =>
            {
            ...
            PEB.OnRestart += (BruteForce sender, object[] pass) => OnThreadRestart?.Invoke(this, sender, pass);
            PEB.OnStart += (BruteForce sender) => OnThreadStart?.Invoke(this, sender);
            PEB.OnStop += (BruteForce sender, object[] pass) =>
            {
                OnThreadStop?.Invoke(this, sender, pass);
                stopCount();
            };
            PEB.OnEnd += (BruteForce sender, object[] pass, bool result) =>
            {
                if (result)
                {//PEB has managed to find a password!
                ...
                else
                {
                    ...
                    {//The Stop command has already been issued.
                        stopCount();
                        return;
                    }
        ...
        */
        ...
        public override bool DoWork(bool waitForWork = false)
        {
            ...
            foreach (PermutationBrute PEB in list) initializeBruteFoceListEvents(PEB);
            started = true;
            OnStart?.Invoke(this);
            delegate_OnStopOrEnd onStopOrEndOrError = (p1, p2) => started = false;
            ...
            foreach (PermutationBrute PEB in list)
            {
                threads.Add(new Thread(() => PEB.Start(BruteForce.ErrorHandlingType.Event)));
